...
#import "AFHTTPSessionManagerExtend.h"
...
//#import "RetStatusEntity.h"
//#import "ResponseCodeAnno.h"
//#import "ResponseCodeConst.h"
//#import "ResponseErrrorHandler.h"
...
@interface AFHTTPSessionManagerExtend ()
@property (readwrite, nonatomic, strong) NSURL *baseURL;
@end

@implementation AFHTTPSessionManagerExtend
...
#pragma clang diagnostic pop
    ...
    dataTask = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * __unused response, id responseObject, NSError *error) {
        if (error) {
            RetStatusEntity *status = [[RetStatusEntity alloc] init];
            if (responseObject!=nil) {
                status.retCode =[[Comm sharedInstance] isNullObject:[responseObject objectForKey:@"errCode"]];
                status.errMsg = [ResponseCodeAnno msgWithResposeCode:[status.retCode integerValue]];
                
                if ([status.retCode intValue] == UMS_UN_AUTHORITY && [[ResponseErrrorHandler sharedInstance] handleLoginSessionError]) {
                    // 登录会话超时，尝试重登录
                    return ;
                }
                
                // 交易MAC校验失败,需要设备进行重新签到.继续提示相关错误信息
                if ([status.retCode intValue] == UMS_CHECK_MAC_ERROR) {
                    [[ResponseErrrorHandler sharedInstance] handleCheckMacError];
                }
            }
            if (error.code==-1009) {
                status.retCode =@"-1009";
                status.errMsg =@"网络异常，请检查网络";
            }
            if (failure) {
                failure(dataTask,status,error);
            }
        } else {
            if (success) {
...
- (id)copyWithZone:(NSZone *)zone {
    AFHTTPSessionManagerExtend *HTTPClient = [[[self class] allocWithZone:zone] initWithBaseURL:self.baseURL sessionConfiguration:self.session.configuration];
